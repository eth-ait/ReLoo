...
class SMPLServer(torch.nn.Module):
    ...
    def __init__(self, gender='neutral', betas=None, v_template=None):
        ...
        smpl_torso_body_v_idx_file = os.path.join(hydra.utils.to_absolute_path(''), '../assets/smpl_torso_body_v_idx_new_new.npy')
        self.smpl_torso_body_v_idx = torch.tensor(np.load(smpl_torso_body_v_idx_file)).cuda()
    ...
    def forward(self, scale, transl, thetas, betas, absolute=False):
        ...
        return {
            'smpl_verts': verts * scale.unsqueeze(1) + transl.unsqueeze(1) * scale.unsqueeze(1),
            "smpl_jnts": joints * scale.unsqueeze(1) + transl.unsqueeze(1) * scale.unsqueeze(1),
            "smpl_tfs": tf_mats,
            "smpl_weights": smpl_output.weights,
            "scale": scale,
        }