...
class ImplicitNet(nn.Module):
    def __init__(self, opt):
        ...
        self.dim_in = opt.d_in
        self.dim_out = opt.d_out + opt.feature_vector_size
        ...
        elif self.cond == "semantic" and opt.dim_semantic_encoding > 0:
            self.cond_layer = [0]
            self.lin_p0 = nn.Linear(256, opt.dim_semantic_encoding)
            self.cond_dim = opt.dim_semantic_encoding
    ...
    def forward(self, input, cond, time_enc=None):
        ...
        out_dim = self.opt.d_out + self.opt.feature_vector_size
...
class RenderingNet(nn.Module):
    ...
    def forward(self, points, normals, view_dirs, body_pose, feature_vectors, frame_latent_code=None):
        ...
        if self.mode == 'nerf_frame_encoding':
            ...
            body_pose = body_pose.unsqueeze(1).expand(*points.shape[:-1], -1)
