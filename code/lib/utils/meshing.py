...
def simplify_mesh(mesh, n_vert=1000):
    try: # if Open3D is installed
        return mesh.simplify_quadric_decimation(n_vert)
    except:
        import pyfqmr
        simplifier = pyfqmr.Simplify()
        simplifier.setMesh(mesh.vertices, mesh.faces)
        simplifier.simplify_mesh(target_count=n_vert)
