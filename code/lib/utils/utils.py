...
def edge_sampling(data, num_sample, *args, **kwargs):
    mask = data["object_mask"].astype(np.float32)
    mask_o = cv2.dilate(mask, np.ones((32, 32), np.uint8))
    mask_i = cv2.erode(mask, np.ones((32, 32), np.uint8))
    mask_e = mask_o - mask_i

    mask = mask.reshape(-1)
    mask_e = mask_e.reshape(-1)
    mask_loc, *_ = np.where(mask > 0)
    edge_loc, *_ = np.where(mask_e > 0)
    mask_idx = np.random.randint(0, len(mask_loc), int(num_sample * 0.6))
    edge_idx = np.random.randint(0, len(edge_loc), int(num_sample * 0.3))
    rand_idx = np.random.randint(0, len(mask), num_sample - len(mask_idx) - len(edge_idx))
    ...
    for k, v in data.items():
        if v.ndim == 3:
            output[k] = v.reshape(-1, v.shape[2])[indices]
        else:
            output[k] = v.reshape(-1)[indices]
    
    inside = np.arange(len(indices))[mask[indices] > 0]
    outside = np.arange(len(indices))[mask[indices] == 0]

    return output, inside, outside
...
def compute_gradient(inputs, outputs, create_graph=True, retain_graph=True):
